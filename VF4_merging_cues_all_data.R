...
sp_VF4_Audio <- st_as_sf(VF4_Audio, coords = c("lon", "lat"), crs = 4326, agr = "constant")
head(sp_VF4_Audio)
str(sp_VF4_Audio)
# plot it 
sp_VF4_Audio_geom <- st_geometry(sp_VF4_Audio)
plot(sp_VF4_Audio_geom, col = "grey") 


#transfor the cattle data so its in the same data frame as the paddock boundary
sp_VF4_Audio_trans <- st_transform(sp_VF4_Audio, crs = 28354)
head(sp_VF4_Audio_trans)
head(eden_valley)
plot(st_geometry(sp_VF4_Audio_trans))
sp_VF4_Audio_trans_geo <- st_geometry(sp_VF4_Audio_trans)#data frame that is just points no attributes
...
ggplot() +
  ...
  geom_sf(data = sp_VF4_Audio_trans_geo)


############################   Now clip ################################################################

sp_VF4_Audio_trans_clip <- st_intersection(sp_VF4_Audio_trans, eden_valley) #message about assumed spatial consistant

#check I have done what I want Looks good

plot(st_geometry(sp_VF4_Audio_trans_clip))
ggplot() +
  geom_sf(data = eden_valley, color = "black", fill = NA)+
  geom_sf(data = sp_VF4_Audio_trans_clip)
...
#bring in the VF 
fence4 <- st_read("Fence4_actual.shp")
st_crs(fence4) <- 28354
st_crs(fence4)
...
sp_VF4_Audio_trans_clip <- mutate(sp_VF4_Audio_trans_clip, 
                                    dist = st_distance(sp_VF4_Audio_trans_clip, fence4))
head(sp_VF4_Audio_trans_clip)
ggplot() +
  geom_sf(data = eden_valley, color = "black", fill = NA) +
  geom_sf(data = fence4, color = "green", fill = NA)+
  geom_sf(data = sp_VF4_Audio_trans_clip)

head(sp_VF4_Audio_trans_clip$dis)  
class(sp_VF4_Audio_trans_clip$dis)
sp_VF4_Audio_trans_clip$dist <- as.double(sp_VF4_Audio_trans_clip$dist)
...
sp_VF4_Audio_clip_animalID <- mutate(sp_VF4_Audio_trans_clip,
                                        animal_ID = case_when(
                                          collar_ID == "ac138" ~ "Q46",
                                          collar_ID == "ac187" ~ "Q36",
...
head(sp_VF4_Audio_clip_animalID)
with(sp_VF4_Audio_clip_animalID, table(date, animal_ID))

#the location of the NA
#NA_sp_VF4_Audio_clip_animalID <- filter(sp_VF4_Audio_clip_animalID,
 #                                          animal_ID == "NA")
#with(NA_sp_VF4_Audio_clip_animalID, table(date, collar_ID))

summary(sp_VF4_Audio_clip_animalID)
...
head(sp_VF4_Audio_clip_animalID)
str(sp_VF4_Audio_clip_animalID)
dim(sp_VF4_Audio_clip_animalID)



# which points fall inside a polygon? Create a new clm for this non_graz when true its in the non grazing zone
sp_VF4_Audio_clip_animalID <- mutate(sp_VF4_Audio_clip_animalID,
                                        non_graz = apply((st_intersects(sp_VF4_Audio_clip_animalID, 
                                                                        VF4_NonGraz, sparse = FALSE)), 1, any))
head(sp_VF4_Audio_clip_animalID)
sp_VF4_Audio_clip_animalID_TRUE <- filter(sp_VF4_Audio_clip_animalID, non_graz == TRUE)
sp_VF4_Audio_clip_animalID_FALSE <- filter(sp_VF4_Audio_clip_animalID, non_graz == FALSE)
ggplot() +
  geom_sf(data = eden_valley, color = "black", fill = NA) +
  geom_sf(data = fence4, color = "green", fill = NA)+
  #geom_sf(data = sp_VF4_Audio_clip_animalID_FALSE)
  geom_sf(data = sp_VF4_Audio_clip_animalID_TRUE)
...
head(sp_VF4_Audio_clip_animalID)
##########       Final output here is  sp_VF4_Audio_clip_animalID                        ####################
st_write(sp_VF4_Audio_clip_animalID, "sp_VF4_Audio_clip_animalID.csv", layer_options = "GEOMETRY=AS_XY")
...
VF_4_InclusionBord_Audio <- rbind(sp_VF4_InclusionBord_animalID,
                                  sp_VF4_Audio_clip_animalID)


head(VF_4_InclusionBord_Audio)

Aduio_sum4 <- group_by(VF_4_InclusionBord_Audio, date, animal_ID, event) %>% 
  summarise(count = n())
...
head(sp_VF4_Pluse_clip_animalID)
##########       Final output here is  sp_VF4_Audio_clip_animalID                        ####################
st_write(sp_VF4_Pluse_clip_animalID, "sp_VF4_Pluse_clip_animalID.csv", layer_options = "GEOMETRY=AS_XY")
...
VF_4_InclusionBord_Pulse <- rbind(sp_VF4_InclusionBord_animalID,
                                  sp_VF4_Pluse_clip_animalID)


head(VF_4_InclusionBord_Pulse)

Pulse_sum4 <- group_by(VF_4_InclusionBord_Pulse, date, animal_ID, event) %>% 
  summarise(count = n())
...
VF_4_Audio_Pulse <- rbind(sp_VF4_Audio_clip_animalID,
                                  sp_VF4_Pluse_clip_animalID)

##########       Final output here is  VF_4_Audio_Pulse                        ####################
st_write(VF_4_Audio_Pulse, "VF_4_Audio_Pulse.csv", layer_options = "GEOMETRY=AS_XY")
st_write(VF_4_Audio_Pulse, "VF_4_Audio_Pulse.shp")
