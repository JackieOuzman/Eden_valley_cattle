...
VF2_incur_event <- mutate(VF1_incur_event,
                                     hms = hms::as.hms(time, tz="GMT"))
#assign  coords for each of the VF dataframes
...
sp_VF1_2_3_4_incur_event <- r

```

```{r inspect the exclusion data}
#head(sp_VF1_incur_event)
#table(sp_VF1_incur_event$animal_ID)
#check_out_NA <- filter(sp_VF1_incur_event, animal_ID == "NA")
#head(check_out_NA)
...
head(event_sum_VF1)
#can I change the date to a factor and then create a box plot?
event_sum_VF1$date_factor <- factor(event_sum_VF1$date)


  ggplot(event_sum_VF1, aes(x = date_factor, y = max_dist))+
  #geom_point()+
  #geom_boxplot(alpha=0.2)+
...
Lets bring in all the data for incur events
