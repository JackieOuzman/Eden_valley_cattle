...
```{r plot the audio data merge data per animal per day}
...
### Audio 
filter(VF1_2_3_4buff_cue_animalID_per_animal_per_day, audio_pulse_started == "Audio") %>% 
...
VF1_2_3_4_cue_animalID_per_day <- group_by(VF1_2_3_4_cue_animalID, date,  audio_pulse_started) %>% 
  summarise(count = n()) 


##as bar per animal per day
ggplot(VF1_2_3_4_cue_animalID_per_day, aes(x = date, y = count))+
  geom_col()+
  #facet_wrap(date ~ animal_ID)+
  theme_bw()+
  geom_text(aes(label = count), vjust = -1.0, hjust = 0.5)+
    theme_classic()+
    labs(x= "Type of cue",
